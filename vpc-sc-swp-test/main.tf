...
resource "google_compute_project_metadata" "default" {
  ...
  depends_on = [ google_project_iam_member.sa_roles, google_access_context_manager_service_perimeter_ingress_policy.ingress_policy ]
}
...
resource "google_access_context_manager_service_perimeter_ingress_policy" "ingress_policy" {
  provider    = google.impersonation
  perimeter = "${google_access_context_manager_service_perimeter.service_perimeter.name}"
  ingress_from {
...
module "google-infra-firewall" {
  ...
  namesuffix  = local.suffix_nodash
  depends_on = [ module.google-infra-vpc, google_project_iam_member.sa_roles, google_access_context_manager_service_perimeter_ingress_policy.ingress_policy ]
}
...
resource "google_compute_instance" "testvms" {
  ...
  metadata            = {
      startup-script      = templatefile("./debian-11-client.sh.tftpl", {
          ...
          swp-fqdn        = "https://abc.local"
          #swp-fqdn        = "https://${trimsuffix(values(google_dns_record_set.swp_proxy_record_set)[0].name, ".")}:${one(google_network_services_gateway.default.ports)}"
      })
  }
...
#resource "google_privateca_ca_pool_iam_binding" "ca_binding" {
#  for_each = toset([google_privateca_ca_pool.swp_tlsinsp_rootca_pool.id, google_privateca_ca_pool.swp_tlsinsp_ca_subpool.id])
#  ca_pool = each.value
