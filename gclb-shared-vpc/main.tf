...
resource "google_compute_project_metadata" "project_metadata" {
    ...
    metadata = {
        enable-oslogin  = "TRUE"
        "gclb-shared-vpc${local.suffix}-cwd" = path.cwd
    }
}
...
resource "google_compute_backend_service" "sp_backend_svc" {
    provider                = google-beta
...
resource "google_compute_url_map" "urlmap_sp" {
    provider        = google-beta
    name            = "urlmap${local.suffix}"
    project         = var.project-id-sp02
    default_service = google_compute_backend_service.sp_backend_svc.id
    host_rule {
        hosts = ["*"]
...
resource "google_compute_target_http_proxy" "http_proxy" {
    ...
    url_map         = google_compute_url_map.urlmap_sp.id
}
...
resource "google_compute_global_forwarding_rule" "xlb_fr" {
    provider                = google-beta
    project                 = var.project-id-sp02
    ...
    ip_address              = google_compute_global_address.xlb_ip.id
}
