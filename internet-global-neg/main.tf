...
resource "google_compute_global_network_endpoint_group" "neg_fqdn_port" {
    name                    = "fqdn-port-neg${local.suffix}"
    network_endpoint_type   = "INTERNET_FQDN_PORT"
    default_port            = 80
}

resource "google_compute_global_network_endpoint" "global_fqdn_ip_port" {
    global_network_endpoint_group = google_compute_global_network_endpoint_group.neg_fqdn_port.name
    fqdn       = "www.google.com"
    port       = 80
}

resource "google_compute_backend_service" "default" {
    name                    = "internet-neg-bs${local.suffix}"
    protocol                = "HTTP"
    load_balancing_scheme   = "EXTERNAL_MANAGED"
    backend {
        group           = google_compute_global_network_endpoint_group.neg_fqdn_port.id
...
resource "google_compute_global_address" "fr_address" {
...
resource "google_compute_global_forwarding_rule" "default" {
    ...
    ip_address              = google_compute_global_address.fr_address.address
}
